  ...
  pub fn new(
    ...
    for i in 0..(pool.pool_count as usize) {
      ...
      match pool.pool[i] {
        ...
        ConstantInfo::Fieldref { .. } => v.push(ConstantInfoRunTime::Fieldref(FieldRef::new(
          Rc::downgrade(&cp),
          &pool,
          pool.pool[i].clone(),
        ))),
        ConstantInfo::Methodref { .. } => v.push(ConstantInfoRunTime::Methodref(MethodRef::new(
          Rc::downgrade(&cp),
          &pool,
          pool.pool[i].clone(),
        ))),
        ConstantInfo::InterfaceMethodref { .. } => v.push(ConstantInfoRunTime::InterfaceMethodref(
